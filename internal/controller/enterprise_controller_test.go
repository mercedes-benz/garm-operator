...
func TestEnterpriseReconciler_ReconcileNormalWithNamespaceFiltered(t *testing.T) {
	mockCtrl := gomock.NewController(t)
	defer mockCtrl.Finish()
	...
	tests := []struct {
		...
		{
			name: "enterprise exist in another namespace - ignore for reconcile",
			object: &garmoperatorv1alpha1.Enterprise{
				ObjectMeta: v1.ObjectMeta{
					Name:      "existing-enterprise",
				...
				Status: garmoperatorv1alpha1.EnterpriseStatus{
					ID: "a850d11c-35d6-4ddf-893d-0d203b8d66ae",
				},
			},
			expectedObject: &garmoperatorv1alpha1.Enterprise{
				ObjectMeta: v1.ObjectMeta{
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
				MetricsBindAddress: ":0", // 0 means "pick a free port"
				Port:               0,    // 0 means "pick a free port"
				Cache: cache.Options{
					Namespaces: []string{"default", "foobar"},
				},
			})
			if err != nil {
				t.Errorf("unable to start manager = %v", err)
				return
			}
