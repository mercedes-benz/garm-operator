...
all_files=()
export IFS=$'\n'
while IFS='' read -r line; do all_error_files+=("$line"); done < <(git ls-files | grep -v -E '\.jpeg$|\.jpg$|\.png$|\.yaml$|^(go.sum|LICENSE|hack/boilerplate.go.txt)$|.*zz_generated.deepcopy.go$')
unset IFS
