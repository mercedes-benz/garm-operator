...
func (i *ImageValidator) ValidateDelete(ctx context.Context, obj runtime.Object) (admission.Warnings, error) {
	...
	numberPools, err := attachedPools(ctx, image.Name)
	if err != nil {
...
func attachedPools(ctx context.Context, imageName string) (int, error) {
	var pools PoolList
	var numberPools []Pool
	if err := c.List(ctx, &pools); err != nil {
		return len(numberPools), err
	}
	...
	for _, pool := range pools.Items {
		...
		if pool.GetDeletionTimestamp() == nil {
			if pool.Spec.ImageName == imageName {
				numberPools = append(numberPools, pool)
			}
		}
	}

	return len(numberPools), nil
}
